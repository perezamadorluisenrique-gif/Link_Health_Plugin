...
- **[Settings] Hardcoded English strings break the Spanish locale.** The "Reglas de corrección automática" empty-state hint ("No rules defined. Rules let the scanner automatically rewrite broken URLs during scans.") and the entire "Email Notifications" section (headings, field labels, helper text, button) render in English while every other string on the same page is correctly localized (es_ES). On a Spanish-locale site this reads as broken/unfinished, directly undercutting "unified system" — one page visibly stitched from two different translation states. Fix: wrap these strings in `__()`/`_e()` with the `native-link-health` text domain, add to the `.pot`, translate in `native-link-health-es_ES.po`, regenerate the `.mo`.
